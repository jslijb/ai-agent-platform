"""
用 baostock 正确 API 补充五粮液数据
baostock.query_profit_data 返回列:
code,year,quarter,pubDate,statDate,roe,netProfitMargin,grossProfitRate,netProfit,eps,revenue,operCost
(注意：不同版本列顺序可能不同，先打印列确认)
"""
import sys
import os
...
def inspect_columns():
    """先检查 baostock 返回的列"""
    lg = bs.login()
    rs = bs.query_profit_data(code="sz.000858", year=2023, quarter=4)
...
def fetch_correct_profit(stock_code, year):
    """获取利润表 - 使用字段名映射"""
    lg = bs.login()
    ...
    while rs.error_code == '0' and rs.next():
        ...
        data = {
            'revenue': sf('revenue'),
            'operating_cost': sf('operCost'),
            'net_profit': sf('netProfit'),
            'eps': sf('eps'),
            'roe': sf('roe'),
            'gross_margin': sf('grossProfitRate'),
            'net_margin': sf('netProfitMargin'),
        }
        break
...
def fix_wuliangye():
    ...
    print("=== 检查 baostock 列结构 ===")
    inspect_columns()
    
    print("\n=== 修复五粮液(000858) 2018-2023 ===")
    for year in range(2018, 2024):
        income = fetch_correct_profit('000858', year)
        if income and income.get('revenue') and income['revenue'] > 1000000:
            ...
            cur.execute("""
                ...
                SET revenue = %s, operating_cost = %s, net_profit = %s, eps = %s,
            ...
            """, (income['revenue'], income.get('operating_cost'), income.get('net_profit'),
                  income.get('eps'), income.get('gross_margin'), income.get('net_margin'), year))
    ...
    for code in ['000066', '600436', '600521', '601186', '601868']:
        for year in [2024, 2025]:
            income = fetch_correct_profit(code, year)
...
def verify():
    ...
    total_roe = cur.fetchone()['cnt']
    print(f"  financial_indicators 有 roe: {total_roe}/26")
