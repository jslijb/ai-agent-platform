...
def fetch_income_data(stock_code, year, quarter):
    """从 baostock 获取利润表数据"""
    ...
    # baostock 使用 sh/sz 前缀
    bs_code = f"sh.{stock_code}" if stock_code.startswith('6') else f"sz.{stock_code}"
    
    # quarter mapping: annual -> 4
    rq = f"{year}-12-31" if quarter == 'annual' else f"{year}-{int(quarter)*3:02d}-30"
    
    fields = "year,quarter,code,revenue,operCost,operateProfit,totalProfit,netProfit,netProfitAttr,eps,bps,grossProfitRate,netProfitMargin"
    
    rs = bs.query_profit_data(code=bs_code, year=year, quarter=4 if quarter == 'annual' else int(quarter))
    ...
    while rs.error_code == '0' and rs.next():
        ...
        data = {
            ...
            'operating_cost': float(row[4]) if row[4] else None,
            'operating_profit': float(row[5]) if row[5] else None,
            'net_profit': float(row[7]) if row[7] else None,
            'net_profit_attributable': float(row[8]) if row[8] else None,
            'eps': float(row[9]) if row[9] else None,
            'bvps': float(row[10]) if row[10] else None,
            'gross_margin': float(row[11]) if row[11] else None,
            'net_margin': float(row[12]) if row[12] else None,
        }
        break
...
def fetch_balance_data(stock_code, year, quarter):
    ...
    rs = bs.query_balance_data(code=bs_code, year=year, quarter=4 if quarter == 'annual' else int(quarter))
    
    data = None
    while rs.error_code == '0' and rs.next():
        row = rs.get_row_data()
        data = {
            'total_assets': float(row[3]) if row[3] else None,
            'total_liabilities': float(row[4]) if row[4] else None,
            'total_equity': float(row[5]) if row[5] else None,
            'current_assets': float(row[6]) if row[6] else None,
            'non_current_assets': float(row[7]) if row[7] else None,
            'current_liabilities': float(row[8]) if row[8] else None,
            'non_current_liabilities': float(row[9]) if row[9] else None,
        }
        break
...
def fetch_cashflow_data(stock_code, year, quarter):
    """从 baostock 获取现金流量表数据"""
    import baostock as bs
    lg = bs.login()
    if lg.error_code != '0':
        return None
    ...
    rs = bs.query_cash_flow_data(code=bs_code, year=year, quarter=4 if quarter == 'annual' else int(quarter))
    
    data = None
    while rs.error_code == '0' and rs.next():
        row = rs.get_row_data()
        data = {
            'operating_cash_flow': float(row[3]) if row[3] else None,
            'investing_cash_flow': float(row[4]) if row[4] else None,
            'financing_cash_flow': float(row[5]) if row[5] else None,
        }
        break
...
def supplement_data():
    ...
    for year in range(2018, 2024):
        income = fetch_income_data('000858', year, 'annual')
        if income and income.get('revenue'):
            print(f"  {year}: revenue={income['revenue']}, net_profit={income['net_profit']}, eps={income['eps']}")
            cur.execute("""
                UPDATE financial_income
                SET revenue = %s, operating_cost = %s, operating_profit = %s,
                    net_profit = %s, "netProfitAttributable" = %s, eps = %s, bvps = %s,
            ...
            """, (income['revenue'], income.get('operating_cost'), income.get('operating_profit'),
                  income.get('net_profit'), income.get('net_profit_attributable'),
    ...
    print("\n=== 补充资产负债表缺失数据 ===")
    missing_bs = [
        ('601319', 2024), ('601319', 2025), ('601555', 2023)
    ]
    for stock_code, year in missing_bs:
        bs_data = fetch_balance_data(stock_code, year, 'annual')
        if bs_data:
    ...
    for row in missing_eps:
        income = fetch_income_data(row['stock_code'], row['report_year'], 'annual')
...
def verify():
    ...
    cur.execute("""
        SELECT 'financial_income' as tbl, COUNT(*) as total,
            ...
            COUNT(CASE WHEN net_profit IS NULL THEN 1 END) as null_np,
            COUNT(CASE WHEN eps IS NULL THEN 1 END) as null_eps
        FROM financial_income
        UNION ALL
        SELECT 'financial_balancesheet', COUNT(*),
            COUNT(CASE WHEN total_assets IS NULL THEN 1 END),
            COUNT(CASE WHEN total_liabilities IS NULL THEN 1 END),
            COUNT(CASE WHEN total_equity IS NULL THEN 1 END)
        FROM financial_balancesheet
        UNION ALL
        SELECT 'financial_indicators', COUNT(*),
            COUNT(CASE WHEN roe IS NULL THEN 1 END),
            COUNT(CASE WHEN roa IS NULL THEN 1 END),
            COUNT(CASE WHEN gross_margin IS NULL THEN 1 END)
        FROM financial_indicators
    """)
    print("\n=== 验证结果 ===")
    for row in cur.fetchall():
        print(f"  {row['tbl']}: total={row['total']}, null_col1={row['null_revenue']}, null_col2={row['null_np']}, null_col3={row['null_eps']}")
    
    cur.close()
    conn.close()

if __name__ == "__main__":
    if not check_baostock():
