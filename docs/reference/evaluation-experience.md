...
## 五-C、V13-r4 经验（数据质量修复，2026-08-01）

### 背景
V13-r3 识别出 PostgreSQL 数据质量是 CR 达标硬阻塞后，逐家修复 PDF 提取问题，重跑 L1/L3/L4 共 55 条评估。

### 结果

| 分类 | V13-r3 综合 | V13-r4 综合 | 变化 | V13-r4 CP | V13-r4 CR | V13-r4 F | V13-r4 AR |
|------|-----------|-----------|------|---------|---------|---------|---------|
| L1-事实提取 | ~0.82 | ~0.96 | +0.18 ✅ | 0.9333 | 0.9333 | 1.0000 | 0.9600 |
| L3-计算推理 | ~0.60 | ~0.62 | +0.02 | 0.1333 | 0.4889 | 0.9778 | 0.8400 |
| L4-趋势分析 | ~0.79 | ~0.84 | +0.15 ✅ | 1.0000 | 0.4500 | 1.0000 | 1.0000 |

**综合 0.8688（首次达标 ≥0.82）**，较 V13-r3 +0.0989。

总体指标变化：
| 指标 | V13-r3 | V13-r4 | 变化 | 达标 |
|------|--------|--------|------|------|
| CP | 0.5636 | 0.7273 | +0.1637 | ❌(受L3拖累) |
| CR | 0.5515 | 0.7242 | +0.1727 | ❌(受L3/L4拖累) |
| F | 0.9939 | 0.9939 | 持平 | ✅ |
| AR | 0.8291 | 0.9345 | +0.1054 | ✅ |
| 综合 | 0.7699 | 0.8688 | +0.0989 | ✅ |

### 好的经验

1. **数据质量是 RAG 系统的基石**：
   - V13-r3 的 L1 CR=0.6667 → V13-r4 的 L1 CR=0.9333（+0.27），完全归因于数据质量修复
   - 华海药业修复后，L1-009 从 CR=0 变为 CR=1.0
   - 教训：结构化数据查询路径下，数据质量直接决定 CR 上限

2. **附注列扫描范围需覆盖全部行**：
   - 华海药业 Page 110 有2个表格（资产负债表尾部27行+利润表起始13行）
   - 合并后前20行全是资产负债表行（无"附注"表头），利润表表头在第28行
   - 修复：_identify_skip_columns 扫描范围从 rows[:20] 改为 rows（全部行）
   - 教训：pdfplumber 的 extract_tables 会返回页面内所有表格，合并后表头位置不可预测

3. **回归测试保障修改安全**：
   - 修改 _identify_skip_columns 后，用 --dry-run 重新提取中国能建/江苏银行/片仔癀
   - 对比关键字段（revenue/net_profit）与数据库一致，确认未破坏其他公司
   - 教训：扩大扫描范围可能影响性能或误判，必须回归测试

4. **L1 全指标达标证明 R001 架构正确**：
   - L1 CP=0.9333, CR=0.9333, F=1.0, AR=0.96
   - SQL 精确查询 + 数据质量 = L1 事实提取类问题最优解
   - 教训：对于数值类事实查询，SQL 路径远优于向量检索

### 踩坑经验

1. **SQL JSON context 对 CP 评估仍不友好**：
   - L3 CP=0.1333 与 V13-r3 持平，未改善
   - 根因：CP 评估的是"检索片段排序质量"，SQL JSON 格式被 LLM 判定为"不相关片段"
   - 优化方向：把 SQL JSON 转成自然语言描述

2. **L4 CR=0.45 同比数据格式问题**：
   - L4 的 CP=1.0, AR=1.0（满分），但 CR=0.45
   - 根因：revenue_yoy 字段格式可能不被 LLM 理解为"同比变化"
   - 优化方向：检查 revenue_yoy 的 SQL 返回格式

3. **LLM 偶尔错误拒绝**：
   - L1-029（东吴证券经纪业务收入）和 L3-015（中国铁建净资产收益率）出现错误拒绝
   - 根因：LLM 在 SQL context 足够时仍偶尔回答"无法回答"
   - 优化方向：system prompt 增加明确约束"当 SQL context 有数据时必须回答"

4. **AGNES API key 失效**：
   - 评估期间 AGNES 返回 401（无效令牌），降级到百炼 qwen-plus
   - 百炼多个版本额度耗尽（2025-07-14/04-28/01-25），最终用 2025-09-11
   - 教训：评估前需检查 LLM provider 可用性

### 下一步优化方向

1. **P0 优化 L3 CP**：SQL JSON context → 自然语言描述（预期 CP 从 0.13→0.80+，综合从 0.87→0.90+）
2. **P1 优化 L4 CR**：检查 revenue_yoy 格式，确保 LLM 能理解同比数据
3. **P1 处理中国人保**：PyMuPDF 或 OCR 提取 PDF 数值（4 条样本影响）
4. **P2 扩展评估范围**：重跑 L2/L5-L9 全量评估，建立完整基线

---

## 五-D、评估体系系统性风险（2026-08-01 深度分析）

### 背景

用户质疑：修复评估和系统两方面问题后，扩大公司评估时效果是否会大打折扣？经深度分析 qa-golden.json 的生成流程，发现评估数据集存在3类系统性风险。

### 评估数据集生成流程现状

```
qa-golden.json 样本生成流程：
  1. 手工编写 query（如"中国能建2025年营收同比变化多少？"）
  2. 用 query 调用 hybridSearch 向量检索，获取 originalText 片段
  3. 用 LLM 基于 originalText 生成 expectedAnswer（ground_truth）
  4. 评估时：expectedAnswer 直接作为 ground_truth 传给 RAGAS
```

### 3类系统性风险（用户洞察）

#### 风险1：获取错误数据（originalText截取错误片段）

**典型案例 L4-005 中国铁建**：
- query："中国铁建2025年营收同比变化多少？"
- 向量检索返回的 originalText 片段："海外实现营业收入 758.735 亿元、同比增长 15.14%"
- LLM 基于这段错误片段生成 expectedAnswer："同比增长约15.1%"
- 实际财报"主要会计数据"表格：总营收同比 **-3.50%（下降）**
- 影响：LLM 回答正确（-3.50%），但 ground_truth 错误（+15.1%），CR=0.0

**根因**：向量检索按语义相似度排序，可能返回包含"同比增长"关键词的片段（海外营收），而非包含"总营收同比"的权威片段（主要会计数据表格）。

#### 风险2：获取数据不全（originalText截取部分文本）

**典型案例**：originalText 只截取了表格的几行，LLM 看不到表头"本期比上年同期增减(%)"列，可能误判数值含义。

**根因**：向量检索的 chunk_size 限制，大表格可能被截断，LLM 缺失关键上下文。

#### 风险3：获取数据过多（expectedAnswer包含query未问的内容）

**典型案例 L4 全部10个样本**：
- query："中国能建2025年营收同比变化多少？"（只问同比）
- expectedAnswer："中国能建2025年营收约为4529.30亿元，同比增长约3.7%"（包含数值+同比）
- 评估器 CR 逻辑：从 ground_truth 提取2条事实，LLM 只回答1条（同比）→ CR=0.5

**根因**：LLM 生成 expectedAnswer 时倾向于"信息丰富"，但评估器 CR 严格按 ground_truth 事实数计算覆盖率。

### 泛化性风险（用户核心疑问）

**结论：是的，扩大公司评估时效果会大打折扣，远低于V13。**

原因：
1. **向量检索的召回率不稳定**：10家样本公司的财报已人工核对，向量检索能命中正确片段；扩到50家公司时，向量检索可能返回错误片段（如海外营收而非总营收），ground_truth 就会错误。
2. **LLM 生成的 expectedAnswer 质量不可控**：LLM 基于 originalText 生成答案，originalText 错误则答案错误；originalText 不全则答案不全。
3. **评估标准不一致**：部分样本 expectedAnswer 严格匹配query，部分样本 expectedAnswer 额外补充信息，CR 计算不统一。
4. **数据源依赖向量检索**：ground_truth 的数据来源是向量检索结果，而非权威数据源（财报表格或SQL查询结果），存在循环依赖（用检索结果评估检索质量）。

### 修复方案

#### 方案A：评估数据集来源切换（推荐）

将 ground_truth 的数据来源从"向量检索originalText"切换为"权威数据源"：
- 数值类问题（L1/L3/L4）：直接从 PostgreSQL SQL 查询结果生成 expectedAnswer
- 非数值类问题（L5-L9）：从人工标注的权威文档生成 expectedAnswer

**优点**：彻底消除"用检索结果评估检索质量"的循环依赖
**缺点**：需要人工标注，工作量大

#### 方案B：评估前校验（过渡方案）

保留现有生成流程，但在评估前跑 check_ground_truth.py 校验：
- 检查 query 与 expectedAnswer 是否匹配（query只问A，expectedAnswer不应包含B）
- 检查 expectedAnswer 的数值与 originalText 是否一致
- 检查 expectedAnswer 的同比方向与原文是否一致

**优点**：快速实施，能发现明显错误
**缺点**：无法发现"originalText本身错误"的问题（如海外营收片段）

#### 方案C：评估器改造（长期方案）

改造评估器逻辑：
- CR 计算：只评估"query实际问的事实"是否被覆盖，而非 ground_truth 全部事实
- 数据来源标注：expectedAnswer 标注数据来源（SQL/向量/人工），不同来源用不同评估策略

### 建议实施路径

1. **立即（R013）**：修正 L4-005、L4-008 的 ground_truth 错误，修正 L4 评估标准过严
2. **短期（R013）**：建立 qa-golden.json 生成规范，新增 check_ground_truth.py 校验脚本
3. **中期（方案A）**：L1/L3/L4 的 expectedAnswer 改为从 SQL 查询结果生成，消除循环依赖
4. **长期（方案C）**：评估器改造，支持 query-aware 的 CR 计算
