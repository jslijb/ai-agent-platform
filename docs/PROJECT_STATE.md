...
- [进行中-P0] R001 财务数据落 PostgreSQL（spec已审批通过，进入阶段1实施）
  ...
  - 阶段2.3 已完成：10 家样本公司全部回填 PostgreSQL（2026-07-31）
    - ✅ raw_tables 全部入库：10 家共 3142 张原始表格
    - ✅ 标准化指标入库 5 家（3表全有）：片仔癀、华海药业、中国长城、中国铁建、中国能建
    - ⚠️ 部分入库 1 家：五粮液（income 8条但字段值 NULL，字段映射失败）
    - ❌ 标准化指标提取失败 4 家：中国人保、格力电器、江苏银行、东吴证券
      - 银行/保险/证券行业报表格式特殊（非标准利润表/资产负债表），字段映射需定制
      - 这 4 家的 raw_tables 已入库，可通过 financial_raw_tables 整表查询
    - 片仔癀验证值正确：2025 营收 90.01 亿、净利润 21.43 亿、总资产 175.6 亿
    - find_pdf_by_stock_code 已修复：支持多目录搜索（根目录+2025_annual）+ 多格式匹配（代码/公司名/反向格式）
    - 华海药业修复：_find_statement_pages 新增全页扫描 fallback（标题不在前5行时）
