...
| 服务 | 地址 | 说明 |
|------|------|------|
| 前端界面（Nginx） | http://localhost | 统一入口，登录、对话、文档管理 |
| Next.js 直连 | http://localhost:3000 | 开发调试用 |
| 数据服务 | http://localhost:8001 | A股行情数据 API |
| 健康检查 | http://localhost/api/health | 系统状态监控 |
| MCP 端点 | http://localhost/api/mcp/sse | 工具调用协议 |
...
1. 登录后进入首页，展示功能入口卡片
2. 点击「RAG 智能问答」或「Agent 对话」进入 `/chat` 页面
2. 在输入框中输入问题，例如：
   - "贵州茅台 2025 年营收是多少？"
   - "招商银行的风险控制措施有哪些？"
   - "中国平安的 ROE 趋势如何？"
...
1. 登录后进入首页，点击「文档管理」进入 `/dashboard` 页面
2. 在控制台页面点击「上传文档」按钮
3. 选择本地文件（支持多选）
4. 选择文档类型：
...
在 `/chat` 页面直接输入问题，系统自动路由到合适的 Agent：
...
访问 http://localhost/api/health 可查看系统各组件状态：
