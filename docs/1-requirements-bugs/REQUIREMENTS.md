...
- **验收**：两类拒绝前缀统一，语气委婉
- **进度（2026-08-17）**：✅ 已完成。`src/server/agents/refusal.ts` 定义两条规范话术（COMPLIANCE_REFUSAL / OUT_OF_KNOWLEDGE_REFUSAL）+ `classifyRefusal`/`normalizeRefusal`/`isRefusalAnswer`；simpleAgent 对抗性拦截/投资建议合规拒绝/langgraph complianceNode 全部改用规范话术；系统提示新增规则20要求 LLM 一字不差使用话术；rag-evaluator 的 `isRefusalAnswer` 改用共享 `REFUSAL_PATTERNS`（新增 回答不了/知识储备不足 模式）；`refusal.test.ts` 12 个用例通过
