...
## 问题6：附注表查询路由优化（BM25+向量匹配表名→SQL查表）
...
当前附注表查询走 `table_name ILIKE '%关键词%'` 模糊匹配，用户关键词与表名措辞不一致时匹配不上（如"应收账款账龄分布"≠"应收账款账龄分析"），fallback到向量检索。但向量检索会破坏表格行列结构（切片把表格切成片段），且embedding对表格数据的语义理解不如对文本的理解。

### 方案

给附注表的元数据（表名+表头行+首行数据）建BM25+向量索引，匹配到表名后直接SQL查整表JSON：
...
BM25/向量检索 → 匹配到 table_name="应收账款账龄分析"（相似度0.85）
  ↓
SQL: SELECT table_data FROM raw_tables WHERE stock_code='sh.600036' AND table_name='应收账款账龄分析'
  ↓
返回完整表格JSON（行列结构完整）
```
...
| 维度 | 当前（ILIKE→向量fallback） | 优化后（BM25+向量→SQL） |
|------|--------------------------|----------------------|
| 匹配准确率 | 低（ILIKE只支持子串匹配） | 高（BM25+向量语义匹配） |
| 表格结构 | 被切片破坏 | 完整保留 |
| 查询速度 | 向量检索慢 | SQL查询快 |
| Token消耗 | chunk片段可能丢失行列 | 整表给LLM，信息完整 |

### 实施步骤

1. 为 `financial_raw_tables` 的 `table_name` + `table_data[0]`（表头行）构建BM25索引
2. 为 `table_name` 构建embedding向量索引（pgvector）
3. 修改 `queryRawTables()`：先BM25+向量匹配表名，命中后SQL查整表
4. 评估对比：附注表查询准确率提升多少