...
| **财务报表** | 五粮液 2025年报 PDF | ❌ failed（0 embeddings） |
...
| 问题 | 影响 | 集成测试使用 |
|------|------|------------|
| 五粮液 PDF status=failed | 向量检索无结果 | I8.4: Agent 被告知数据不可用 |
| efinance 格力历史行情 FAIL | 多源降级 | I10.2: 降级到 baostock |
...
| I8.3 | 五粮液 status=failed 时检索降级 | 五粮液 PDF status=failed，0 embeddings | query="五粮液2025年毛利率" | hybridSearch 返回空或仅其他文档结果 | `assert(results.filter(r => r.documentName?.includes("五粮液")).length === 0)` |
...
| I10.2 | 多源降级：efinance失败→baostock | efinance 格力历史行情 FAIL | code="000651", source auto | 自动降级到 baostock，返回约 243 条记录 | `assert(data.length >= 200)`; 日志含 efinance 失败尝试 |
...
| **错误传播正确性** | 错误信息完整传递，下游优雅降级 | I1.5, I2.2, I2.3, I8.3, I9.2, I13.3, I13.4 |
| **降级路径可用性** | 可选依赖不可用时降级到备用方案 | I3.4, I6.1-6.4, I7.2, I8.3, I10.2, I11.5, I12.5 |
